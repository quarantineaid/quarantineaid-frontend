...
export const makeVolunteerSelection = async (payload) => {
    const res = await axios.post(`${BASE_URL}situations/getting-help/accept`, {...payload});
    if (res.statusText.toLowerCase() === 'ok') {
        return true
    } else {
        return false
...
export const requesterStatusUpdate = async (payload) => {
    debugger;
    const res = await axios.post(`${BASE_URL}situations/getting-help/status`, {...payload});
    if (res.statusText.toLowerCase() === 'ok') {
        return true
    } else {
        return false
